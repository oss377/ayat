...
export default function PropertyComparison() {
  ...
  const generateAI = async (homes: Property[]) => {
    setLoadingAI(true);
    try {
      const res = await fetch('/api/ai', { // Corrected API endpoint
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ properties: homes }),
      });
      const { conclusion } = await res.json(); // The backend now returns a 'conclusion' field
        ...
        <div className="p-4 print:hidden">
          <div className="bg-white rounded-xl p-6 shadow-lg">
            ...
            <div className="relative">
              <input readOnly value={window.location.href} className="w-full bg-gray-100 rounded-lg py-3 pl-4 pr-12 text-sm" />
